                ...
                <div className="mx-auto max-w-6xl">
                    ...
                    <div className="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
                        <div>
                            ...
                            <select value={role} onChange={(e) => setRole(e.target.value)} className="mt-1 rounded-lg border-gray-300 text-sm">
                    ...
                    <ActiveFilters
                        filters={[
                            { label: 'Role', value: role, displayValue: role === 'encoder' ? 'Encoder' : 'Admin', onRemove: () => { setRole(''); router.get(route('user-accounts.index'), { status }); } },
                            { label: 'Status', value: status, displayValue: status, onRemove: () => { setStatus(''); router.get(route('user-accounts.index'), { role }); } },
                        ]}
                    ...
                    {users.length > 0 ? (
                        ...
                        <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
                            <EmptyState title="No user accounts found" description="There are no user accounts matching the selected filter criteria." />
                        </div>
                    )}
                </div>
            </div>
